	...
	<div>
		<form action="petListUpdate.do" method="post" enctype="multipart/form-data" >
			...
			<select class="form-control" id="petListState" name="petListState" >
				...
				<option value="입양승인대기" <c:if test="${vo.petListState eq '입양승인대기'}">selected</c:if>>입양대기</option>
				<option value="입양승인완료" <c:if test="${vo.petListState eq '입양승인완료'}">selected</c:if>>입양승인</option>
				<option value="입양승인불가" <c:if test="${vo.petListState eq '입양승인불가'}">selected</c:if>>입양불가</option>
